...
if (isset({dataset}) && !empty({dataset})) {
	...
	$FelCodigoMoneda = strtoupper(strip_tags($resultadoConsulta[0][53]));  // Moneda*
	$FelTipoFrase = strtoupper(strip_tags($resultadoConsulta[0][54]));  // Tipo frase*
	$FelCodigoEscenario = strtoupper(strip_tags($resultadoConsulta[0][55]));  // codigo esenario tipo frase*
	
	$UUIDERR = strtoupper($resultadoConsulta[0][49]);  // nuero documento error por certificado duplicado*
	$FelSerieErro = strtoupper($resultadoConsulta[0][48]);  // nuero serie error por certificado duplicado*
	$FelDocErro = strtoupper($resultadoConsulta[0][47]);  // nuero serie error por certificado duplicado*
        ...
        if (isset($responseArray['RequestTransactionResult'])) {
			...
			//	}
				...
				if (isset($response['Result']) && $response['Result'] == false){
					...
					//fin de insert falla certificacion
                } else {
